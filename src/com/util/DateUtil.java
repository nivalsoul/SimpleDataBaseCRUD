...
public class DateUtil {
     ...
     * @param unit 时间单位（“month”、“day”、“year”、“hour”、“minute”、“second”）
     * @return 计算后的时间
     */
    @SuppressWarnings("deprecation")
    public static Date addDate(Date basicDate, int interval, String unit) {
        Date date = basicDate;
        if (date == null) date = new Date();
        if (interval == 0) return date;
        if (StringUtil.compareTo(unit, "year") == true) {
            date.setYear(date.getYear() + interval);
        } else if (StringUtil.compareTo(unit, "month") == true) {
            int month = date.getMonth() + interval;
            ...
            date.setYear(year);
            date.setMonth(month);
        } else if (StringUtil.compareTo(unit, "day") == true) {
            long time = date.getTime();
            time = time + interval * 24 * 60 * 60 * 1000; // 时间是以毫秒为单位的。
            date.setTime(time);
        } else if (StringUtil.compareTo(unit, "hour") == true) {
            long time = date.getTime();
            time = time + interval * 60 * 60 * 1000; // 时间是以毫秒为单位的。
            date.setTime(time);
        } else if (StringUtil.compareTo(unit, "minute") == true) {
            long time = date.getTime();
            time = time + interval * 60 * 1000; // 时间是以毫秒为单位的。
            date.setTime(time);
        } else if (StringUtil.compareTo(unit, "second") == true) {
            long time = date.getTime();
            time = time + interval * 1000; // 时间是以毫秒为单位的。
            date.setTime(time);
        }
        return date;
    }
    ...
    public static Date getDate(String dateStr, String pattern) {
        if (StringUtil.isEmpty(dateStr) == true) return null;
        SimpleDateFormat b = new SimpleDateFormat(pattern);
        try {
    ...
    /**
     * 返回指定日期时间Timestamp对象的对应日期中文字符串表达式,格式为:yyyy年M月d日 HH时mm分ss秒
     * <br>该方法同getChineseString，只是如果月份和天数以0开头的话则忽略
     * @param java.sql.Timestamp 日期时间Timestamp对象
     * @return String 指定日期时间Timestamp对象的对应字符串表达式
     */
    public static String getShortChineseString(Timestamp tst) throws Exception {
    	if (tst == null) return "";
    	java.text.SimpleDateFormat myFormat = new SimpleDateFormat("yyyy年M月d日 HH时mm分ss秒");
    	Date date = new Date(tst.getTime());
    	return myFormat.format(date);
    }
    ...
    /**
     * 返回指定日期时间Timestamp对象的对应日期中文字符串表达式,格式为:yyyy年M月d日
     * <br>该方法同getChineseDate，只是如果月份和天数以0开头的话则忽略
     * @param tst
     * @return
     * @throws Exception
     */
    public static String getShortChineseDate(Timestamp tst) throws Exception {
        if (tst == null) return "";
        java.text.SimpleDateFormat myFormat = new SimpleDateFormat("yyyy年M月d日");
        Date date = new Date(tst.getTime());
        return myFormat.format(date);
    }
	...
	*/
    ...
    public static ArrayList<String> getStringYMDHMS(String stime) throws Exception {
        stime = StringUtil.transNull(stime);
	 ...
	 * @throws Exception
	 */
	public static int diffDays(Date startTime, Date endTime) throws Exception {
		Calendar s = Calendar.getInstance();
        Calendar e = Calendar.getInstance();
        s.setTime(startTime);
        e.setTime(endTime);
		long c = s.get(Calendar.DAY_OF_YEAR) - e.get(Calendar.DAY_OF_YEAR);
		return (int) c;
	}
	/**
	 * 计算天差（天份数）
	 ...
	 * @throws Exception
	 */
	public static int diffDays(String startTime, String endTime) throws Exception {
		return diffDays(DateUtil.getDateTime(startTime), DateUtil.getDateTime(endTime));
	}
	...
	public static int diffMonth(Date startTime, Date endTime) throws Exception {
        ...
        s.setTime(startTime);
        e.setTime(endTime);
		long c = s.get(Calendar.MONTH) - e.get(Calendar.MONTH);
		return (int) c;
	}
	/**
	 * 计算月份差（月份数）
	 ...
	 * @throws Exception
	 */
	public static int diffMonth(String startTime, String endTime) throws Exception {
		return diffMonth(DateUtil.getDateTime(startTime), DateUtil.getDateTime(endTime));
	}
	 ...
	 * @throws Exception
	 */
	public static int diffYear(Date startTime, Date endTime) throws Exception {
		Calendar s = Calendar.getInstance();
		Calendar e = Calendar.getInstance();
		s.setTime(startTime);
		e.setTime(endTime);
		long c = s.get(Calendar.YEAR) - e.get(Calendar.YEAR);
		return (int) c;
	}
	/**
	 * 计算年份差（年份数）
	 ...
	 * @throws Exception
	 */
	public static int diffYear(String startTime, String endTime) throws Exception {
       return diffYear(DateUtil.getDateTime(startTime), DateUtil.getDateTime(endTime));
	}
	...
	public static int getBirthday(String startTime, String endTime)
		...
		if (diffMonth(startTime, endTime) < 0) {
			return birthday - 1;
		} else if (diffMonth(startTime, endTime) > 0) {
			return birthday;
		} else {
